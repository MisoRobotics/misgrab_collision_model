...
def make_behaviors_table_df(db_string):
...
def _main():
    # sc_db_string = "mysql+mysqldb://logger:tastysoup@localhost/simple_chef_db"
    # cl_db_string = "mysql+mysqldb://logger:tastysoup@localhost/chippy_log"
    sc_db_string = "mysql+mysqldb://root:misosoup@localhost/simple_chef_db"
    cl_db_string = "mysql+mysqldb://root:misosoup@localhost/chippy_log"
...
with dataset as (
    ...
    from behaviors
    WHERE
        FROM_UNIXTIME(start_time) BETWEEN CURRENT_TIMESTAMP - INTERVAL '6' DAY AND CURRENT_TIMESTAMP - INTERVAL '3' DAY
...
with offsets_base as (
    ...
    FROM
        offset_atlas
    WHERE
        offset_atlas_time BETWEEN CURRENT_TIMESTAMP - INTERVAL '20' DAY AND CURRENT_TIMESTAMP - INTERVAL '3' DAY
...
"""
    ...
    behaviors_df = pd.read_sql_query(text(query), con=simple_chef_db_con)
    end_time = time.time()
